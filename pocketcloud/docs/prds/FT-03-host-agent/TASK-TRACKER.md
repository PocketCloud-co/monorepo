...
| HA-017 | P0 | MDM silent-install packages + org enrollment tokens (F20, MVP per DR-08) | HA-011, HA-006, HA-019 | MSI (Intune) + PKG (Jamf) + Linux policy install deploy silently to a 20-device test fleet with zero per-device interaction; enrollment token binds devices to the org pool; verified end-to-end against CP-014 scoping | Proposed | |
| HA-019 | P0 ⚠ | MDM-channel integrity + non-suppressible transparency notice (SEC-001 gaps 2+8) | HA-005, SEC-003 | org policy objects signed by issuer and verified against a platform key independent of the MDM channel; agent enforces monotonic restriction (local tightens only — SPEC-005 §3 invariant test); transparency notice cannot be suppressed by any policy; adversarial test: forged/loosened policy rejected; MUST merge before HA-017 | Proposed | |
| HA-020 | P1 ⚠ | Session KV-cache encryption + wipe-on-close (SEC-001 gap 4; owns FT-06's KV threat row) | HA-003 | KV cache encrypted at rest under the session key; wiped on session close/failover; test: post-session disk/memory scan finds no plaintext residue; referenced by LS-007/LS-R6 | Proposed | |
