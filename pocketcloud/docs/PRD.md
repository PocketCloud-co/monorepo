...
**Mode 2 — The sandwich (flagship).** The customer SDK runs the model's *edges* locally: tokenizer + embedding layer on input, final LM head + sampling on output. The fabric runs only the middle blocks. Consequences: raw tokens never leave the customer in either direction — the fabric streams final hidden states (~10–30 KB/token) back to the SDK, which decodes them locally, so the fabric never learns which words went in **or came out**. Client-side cost is small (embedding/unembedding is a tiny fraction of total weights). **Honesty constraint (stated in all docs and marketing):** this is *transport privacy*, not cryptographic privacy — activation-inversion attacks recover a meaningful fraction of tokens from intermediate activations (~59% at a 2-layer split, ~35% at 8 layers in published attacks; see `FEASIBILITY-GTM.md` §1). Mode 2 raises the attacker's cost and removes plaintext from the wire; only Mode 3 (MPC) carries a mathematical guarantee, and the two must never be conflated in customer-facing claims.
